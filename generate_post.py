...
- [시각 품질 점수] 본문 기반 가독·표현 일관성 점수 산출 (품질 게이트·대시보드 추적용)
...
- [SNS] Blogger 발행 성공 후 Threads 자동 공유 (Secrets 설정 시에만)
...
# --- [NEW] Threads 자동 발행 (Blogger 성공 직후)
...
# [NEW-수익 최적화] 승인 후 여러 광고 단위(인아티클/디스플레이 등)를 콤마로 구분해 순환 배치하고 싶을 때 사용.
# 예: "1234567890,9876543210". 비어 있으면 ADSENSE_SLOT_ID 하나만 사용(기존 동작과 동일, 하위호환).
ADSENSE_SLOT_IDS: List[str] = [s.strip() for s in os.environ.get("ADSENSE_SLOT_IDS", "").split(",") if s.strip()] \
    or ([ADSENSE_SLOT_ID] if ADSENSE_SLOT_ID else [])
# [NEW-수익 최적화] 한 글에 삽입할 최대 광고 슬롯 수. Google 광고 밀도 정책(콘텐츠 대비 과도한 광고 금지)을
# 지키기 위해 섹션(H2) 수에 따라 자동으로 이보다 적게 배치될 수 있다.
ADSENSE_MAX_AD_SLOTS = max(1, int(os.environ.get("ADSENSE_MAX_AD_SLOTS", "2")))
...
# =====================================================================
# [NEW-포맷 다양화] 8가지 글 형식(포맷) — "같은 틀 반복" AdSense 리스크를 구조적으로 낮춘다.
# 5대 콘텐츠 기둥(표현소개/직역문제/실제사용/문화맥락/참여클로징)과 H.O.L.D 흐름, 문화 절대 단정
# 금지 규칙은 형식과 무관하게 항상 그대로 유지되며, 각 포맷은 "실제 사용 장면"을 중심으로 정보를
# *어떻게 보여줄지*(리스트/Q&A/비교표/체크리스트/대화체/타임라인/미신타파 등)만 바꾼다.
# =====================================================================
CONTENT_FORMATS: List[Dict[str, str]] = [
    {
        ...
        "name": "스토리텔링형(내러티브)",
        "instruction": (
            "형식: 스토리텔링형. 소제목 사이 전환을 짧은 장면·대사로 이어 하나의 이야기처럼 읽히게 하세요. "
            "번호 목록이나 표보다는 문단 흐름과 대사 인용으로 서사를 이어가세요."
        ),
    },
    {
        "id": "listicle",
        "name": "리스트형(Listicle)",
        "instruction": (
            "형식: 리스트형. '실제 사용 장면' 섹션(및 필요하면 다른 섹션도)을 <ol><li> 안에 "
            "<strong>짧은 소제목</strong> + 1~2문장 설명 형태로, 번호가 매겨진 3~5개 항목으로 구성하세요."
        ),
    },
    {
        "id": "qna",
        "name": "Q&A형(질문답변)",
        "instruction": (
            "형식: Q&A형. 외국인 학습자가 실제로 물어볼 법한 질문 형태로 H2 소제목을 적고("
            "예: <h2>Does \"표현\" really mean...?</h2>), 그 아래 <p>에 직접 답하듯 설명하세요."
        ),
    },
    {
        "id": "compare",
        "name": "비교형(Dictionary vs Real Meaning)",
        "instruction": (
            "형식: 비교형. '사전적 의미(Dictionary says)'와 '실제 쓰임(Koreans actually mean)'을 "
            "대비시키세요. 짧은 2열 <table><tr><th>Dictionary</th><th>Real meaning</th></tr>...</table> "
            "비교표를 '직역 문제' 섹션 안에 하나 포함하세요."
        ),
    },
    {
        "id": "checklist",
        "name": "체크리스트형",
        "instruction": (
            "형식: 체크리스트형. '실제 사용 장면'을 '이럴 때 써도 될까?' 관점의 체크리스트로 제시하세요. "
            "<ul><li>✅ 자연스러운 상황 설명</li><li>❌ 어색해지는 상황 설명</li></ul> 형태를 활용하세요."
        ),
    },
    {
        "id": "dialogue",
        "name": "대화체 시나리오형",
        "instruction": (
            "형식: 대화체 시나리오형. '실제 사용 장면'에 두 사람(A, B)의 짧은 대화 스크립트를 "
            "<p><strong>A:</strong> ... </p><p><strong>B:</strong> ...</p> 형태로 2~3턴 이상 보여준 뒤, "
            "바로 아래에서 뉘앙스를 해설하세요."
        ),
    },
    {
        ...
        "name": "타임라인형(상황 전개)",
        "instruction": (
            "형식: 타임라인형. 하루의 시간 흐름(아침→점심→저녁) 또는 사건 전개 순서를 따라가며 "
            "표현이 등장하는 순간을 보여주세요. 각 시점을 <strong>시간대/단계</strong>로 표시하세요."
        ),
    },
    {
        "id": "mythbust",
        "name": "미신 타파형(오해 vs 진실)",
        "instruction": (
            "형식: 미신 타파형. '직역 문제' 또는 '문화 맥락' 섹션에서 외국인이 흔히 하는 오해를 "
            "<p><strong>Misconception:</strong> ...</p><p><strong>Reality:</strong> ...</p> 짝으로 "
            "2~3세트 제시하세요."
        ),
    },
]
...
def pick_next_content_format(queue: Dict[str, Any]) -> Dict[str, str]:
    """8가지 글 형식을 '뽑기 주머니(bag)' 방식으로 순환시킨다.
    주머니 안의 8개를 모두 한 번씩 다 쓸 때까지는 같은 형식이 절대 반복되지 않으며,
    주머니가 비어 새로 채울 때도 직전에 쓴 형식이 새 주머니 맨 앞에 바로 이어지지 않도록
    한 번 더 조정한다 (재실행해도 같은 queue 상태면 항상 같은 다음 형식이 나오도록 결정적이지 않고,
    매 실행 랜덤 셔플이되 '연속 중복 없음'만 보장)."""
    bag: List[str] = [f for f in (queue.get("format_bag") or []) if any(c["id"] == f for c in CONTENT_FORMATS)]
    last_format = queue.get("last_format", "")
    all_ids = [f["id"] for f in CONTENT_FORMATS]

    if not bag:
        bag = all_ids[:]
        random.shuffle(bag)
        if last_format and bag[0] == last_format and len(bag) > 1:
            bag.append(bag.pop(0))

    chosen_id = bag.pop(0)
    queue["format_bag"] = bag
    queue["last_format"] = chosen_id
    save_queue(queue)

    return next((f for f in CONTENT_FORMATS if f["id"] == chosen_id), CONTENT_FORMATS[0])


def select_content_format() -> Dict[str, str]:
    """run()에서 제목 선정 경로(자동/수동)와 무관하게 항상 호출되는 진입점."""
    queue = load_queue()
    fmt = pick_next_content_format(queue)
    logger.info(f"[포맷 선택] {fmt['name']} ({fmt['id']}) — 남은 주머니: {load_queue().get('format_bag')}")
    return fmt
...
def load_queue() -> Dict[str, Any]:
    default = {
        ...
        "pending_review": [],
        "ab_title_log": [],
        "format_bag": [],   # [NEW] 8가지 글 형식(포맷) 순환용 "뽑기 주머니"
        "last_format": "",  # [NEW] 직전에 쓴 형식 id (주머니 경계에서 연속 중복 방지)
        "quality_flags": [],  # [NEW] 자동 재생성 실패한 위반 리스크 글 목록(수동 확인 필요)
    }
...
<table>
<tr><th>날짜</th><th>제목</th><th>형식</th><th>바로가기</th></tr>
{rows}
</table>
</body>
</html>
...
# =====================================================================
# [NEW-SEO] 로마자 표기 변환 (국립국어원 로마자 표기법 기반 간이 변환기)
# - 해외 학습자는 한글 자체보다 로마자(Gwaenchaneusimyeon 등)로 검색하는 경우가 많다.
# - 연음·구개음화 등 발음 동화 규칙은 적용하지 않는 "음절 단위 직접 변환"이며,
#   완벽한 표준 표기가 아니라 SEO alt 텍스트·검색 키워드 노출용으로 충분한 수준을 목표로 한다.
# =====================================================================
_RR_CHO = ["g", "kk", "n", "d", "tt", "r", "m", "b", "pp", "s", "ss", "", "j", "jj", "ch", "k", "t", "p", "h"]
_RR_JUNG = ["a", "ae", "ya", "yae", "eo", "e", "yeo", "ye", "o", "wa", "wae", "oe", "yo", "u", "wo", "we", "wi", "yu", "eu", "ui", "i"]
_RR_JONG = ["", "g", "kk", "gs", "n", "nj", "nh", "d", "l", "lg", "lm", "lb", "ls", "lt", "lp", "lh", "m", "b", "bs", "s", "ss", "ng", "j", "ch", "k", "t", "p", "h"]


def romanize_korean(text: str) -> str:
    """한글 텍스트를 로마자로 간이 변환한다 (해외 검색 키워드/alt 텍스트용).
    한글 음절이 아닌 문자(로마자, 숫자, 일부 기호)는 그대로 통과시킨다."""
    if not text:
        return ""
    out: List[str] = []
    for ch in text:
        ...
        if 0xAC00 <= code <= 0xD7A3:
            ...
            jong = offset % 28
            out.append(_RR_CHO[cho] + _RR_JUNG[jung] + _RR_JONG[jong])
        elif ch.isspace():
            out.append(" ")
        elif ch in "().,!?·":
            continue
...
def _seo_title_text(pattern: str, expr: str) -> str:
    """'{expr}' / '{roman}' 플레이스홀더가 있는 SEO 제목 패턴에 표현+로마자 표기를 채운다.
    로마자 변환에 실패(빈 값)하면 '({roman})' 구간을 통째로 제거한다."""
    roman = romanize_korean(expr)
    if roman and roman.lower() != expr.lower():
        return pattern.format(expr=expr, roman=roman)
    return re.sub(r"\s*\(\{roman\}\)", "", pattern).format(expr=expr)


def _seo_alt_text(expression: str, title: str) -> str:
    """썸네일/히어로 이미지 alt 텍스트. 로마자 표기를 앞에 두어
    'Gwaenchaneusimyeon Korean expression meaning' 형태의 해외 검색 유입을 노린다."""
    expr = (expression or "").strip() or _extract_expression_from_title(title or "", strict=False)
    roman = romanize_korean(expr) if expr else ""
    if expr and roman:
        return f"{roman} ({expr}) Korean expression meaning"
    return (title or "").strip() or "Korean expression meaning"
...
"""
    resolved_title = title if title else f'"{expr}" Meaning in Korean: What Does It Really Mean?'
    return {
        ...
        "content_format_name": "스토리텔링형(내러티브)",
    }
...
def generate_article(title: str, content_format: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    for attempt in range(1, GEMINI_MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            article["content_format"] = fmt.get("id", "")
            article["content_format_name"] = fmt.get("name", "")
...
_HANGUL_FONT_CACHE: Dict[str, bool] = {}


def _hangul_font_available() -> bool:
    """[NEW-썸네일 안정화] 실제 한글 지원 TTF 파일이 설치되어 있는지 확인(1회 계산 후 캐시).
    apt/curl 폰트 확보가 실패한 러너에서는 PIL이 ImageFont.load_default()(한글 미지원 비트맵
    폰트)로 조용히 대체되어, 한글 텍스트가 빈 사각형(tofu)으로 깨져 보이는 문제가 있었다.
    이 함수가 False를 반환하면 썸네일에 한글을 아예 그리지 않고 로마자만 사용해 이를 원천 차단한다."""
    if "ok" not in _HANGUL_FONT_CACHE:
        _HANGUL_FONT_CACHE["ok"] = any(
            os.path.exists(p) and os.path.getsize(p) > 0 for p in FONT_CANDIDATES
        )
    return _HANGUL_FONT_CACHE["ok"]
...
def _generate_thumbnail_local(title: str, output_path: str, theme: Dict[str, Any], expression: str, category: str = "번역감정") -> None:
    ...
    # [개편-썸네일 안정화] 폰트 설치 여부와 무관하게 항상 렌더링 가능한 로마자(영문) 표현을
    # 메인 헤드라인으로 사용한다. 한글 원문은 실제 한글 지원 폰트가 확인된 경우에만
    # 헤드라인 아래 보조 텍스트로 작게 추가한다 (폰트 실패 시에도 헤드라인 자체는 항상 정상 출력).
    roman_headline = romanize_korean(expression).upper() if expression else ""
    hangul_ok = _hangul_font_available()
    headline = roman_headline or (expression if hangul_ok else "")
    show_kr_line = bool(roman_headline and hangul_ok and expression)

    if headline and len(headline) <= 40:
        headline_font_size = 108 if len(headline) <= 10 else (84 if len(headline) <= 18 else 60)
        headline_font = _load_font(headline_font_size)
        max_text_w = w - 100
        lines = _wrap_by_pixel_width(draw, headline, headline_font, max_text_w)[:2]
        line_h = headline_font_size + 16
        # 영문 한 줄 서브타이틀(빈 썸네일처럼 보이지 않게)
        sub = 'Korean expression · Learn the nuance'
        sub_font = _load_font(28)
        kr_font = _load_font(40) if show_kr_line else None
        kr_line = f"「{expression}」" if show_kr_line else ""
        total_h = line_h * len(lines) + (52 if kr_line else 0) + 36 + 20
        ty = h // 2 - total_h // 2

        text_fill = _blend_rgb((255, 255, 255), accent_rgb, 0.22) + (255,)
        text_stroke = _blend_rgb(accent_rgb, (0, 0, 0), 0.55) + (240,)
        for line in lines:
            lb = draw.textbbox((0, 0), line, font=headline_font)
            tw = lb[2] - lb[0]
            tx = (w - tw) / 2 - lb[0]
            draw.text((tx, ty - lb[1]), line, font=headline_font, fill=text_fill,
                       stroke_width=5, stroke_fill=text_stroke)
            ty += line_h
        if kr_line:
            kb = draw.textbbox((0, 0), kr_line, font=kr_font)
            draw.text(((w - (kb[2] - kb[0])) / 2 - kb[0], ty + 6 - kb[1]), kr_line, font=kr_font,
...
def _manual_ad_unit(slot_id: str = "", unit_key: str = "") -> str:
    """수동 광고 유닛. 심사 모드이거나 클라이언트/슬롯 미설정 시 빈 문자열.
    slot_id를 지정하면 그 슬롯으로, 아니면 ADSENSE_SLOT_ID(단일 슬롯, 하위호환)를 사용한다."""
    if ADSENSE_REVIEW_MODE:
        return ""
    sid = (slot_id or ADSENSE_SLOT_ID or "").strip()
    if not (ADSENSE_CLIENT_ID and sid):
        return ""
    key_attr = f' data-ad-unit-key="{unit_key}"' if unit_key else ""
    return (
        f'<div style="margin:32px 0;text-align:center;clear:both;" class="ad-unit"{key_attr} data-nosnippet>'
        ...
        f'data-ad-slot="{sid}" data-ad-format="auto" data-full-width-responsive="true"></ins>'
        '<script>(adsbygoogle = window.adsbygoogle || []).push({});</script></div>'
    )


def _pick_ad_positions(h2_count: int, max_slots: int) -> List[int]:
    """[NEW-수익 최적화] H2 섹션 개수에 맞춰, Google 광고 밀도 정책을 지키는 안전한 삽입 위치
    (H2 인덱스 목록)를 계산한다.
    규칙: ① 첫 2개 섹션(훅·직역 문제)에는 광고를 넣지 않는다 — 이탈률·심사 리스크 최소화.
          ② 마지막 섹션(참여 클로징) 바로 앞에는 넣지 않는다 — 클로징까지 매끄럽게 읽히게.
          ③ 광고 사이 최소 2개 섹션 간격을 둔다 — 과도한 광고 밀집 방지.
          ④ 섹션이 부족하면(예: H2 3개 이하) 자동으로 슬롯 수를 줄인다(0~1개)."""
    if h2_count <= 0:
        return []
    candidates = [i for i in range(2, h2_count - 1)]
    if not candidates:
        return []
    chosen: List[int] = []
    min_gap = 2
    step = max(min_gap, len(candidates) // max(1, max_slots))
    pos = candidates[0]
    for _ in range(max_slots):
        if pos > candidates[-1]:
            break
        if not chosen or pos - chosen[-1] >= min_gap:
            chosen.append(pos)
        pos += step
...
def insert_manual_ads(article: Dict[str, Any]) -> Dict[str, Any]:
    """[개편-수익 최적화] 리뷰 모드에서는 광고를 전혀 넣지 않아 심사에 집중하고,
    리뷰 모드 해제(승인 후) 시에는 ADSENSE_MAX_AD_SLOTS개까지 광고를 밀도 규칙을 지키며
    분산 배치한다. ADSENSE_SLOT_IDS가 여러 개면 슬롯별로 순환 사용해 광고 단위 성과를
    비교(A/B)할 수 있게 한다."""
    if ADSENSE_REVIEW_MODE or not ADSENSE_SLOT_IDS:
        return article

    body = article.get("html_body", "")
    matches = list(re.finditer(r"<h2\b", body, flags=re.IGNORECASE))
    h2_count = len(matches)
    if h2_count == 0:
        return article

    positions = _pick_ad_positions(h2_count, ADSENSE_MAX_AD_SLOTS)
    if not positions:
        # 섹션이 아주 적은 짧은 글은 마지막에 광고 1개만 (기존 동작과 동일한 폴백)
        ad_html = _manual_ad_unit(ADSENSE_SLOT_IDS[0], "fallback_end")
        if ad_html:
            article["html_body"] = body + ad_html
        return article

    offset = 0
    for slot_i, pos in enumerate(positions):
        ad_html = _manual_ad_unit(ADSENSE_SLOT_IDS[slot_i % len(ADSENSE_SLOT_IDS)], f"pos{pos}")
        if not ad_html:
            continue
        insert_at = matches[pos].start() + offset
        current_body = article.get("html_body", body)
        article["html_body"] = current_body[:insert_at] + ad_html + current_body[insert_at:]
        offset += len(ad_html)
...
def _wrap_webtoon_panels(html_body: str, theme: Dict[str, Any]) -> str:
    ...
    for i, section in enumerate(sections, start=1):
        ...
        # 사용 예시 목록이 있는 섹션이면 말풍선 카드로 스타일링
        # ([개선] 8가지 형식 중 '리스트형'이 <ol><li>를 쓰므로 <ul>뿐 아니라 <ol>도 함께 인식)
        if re.search(r"<(?:ul|ol)[\s>]", section) and re.search(r"<li[\s>]", section):
            section = re.sub(
                r"<li>(.*?)</li>",
                lambda m: (
            ...
            section = section.replace("<ol>", '<ol style="padding-left:0;margin:14px 0 0;">')
...
def save_post(article: Dict[str, Any]) -> Tuple[Dict[str, Any], str, str, str, str]:
    ...
    title_escaped = html.escape(_seo_alt_text(article.get("expression", ""), title), quote=True)
    ...
    if PUBLISH_GITHUB_PAGES_SITE:
        page_html = POST_TEMPLATE.format(
            ...
            bottom_ad=_manual_ad_unit(ADSENSE_SLOT_IDS[0] if ADSENSE_SLOT_IDS else ""),
    ...
    post_meta = {
        ...
        "content_format_name": article.get("content_format_name", ""),
    }
...
def generate_static_pages() -> None:
    ...
    contact_block = (
        ...
        "<p><b>Email:</b> Please use the contact form linked from our Blogger site, or reach out via social media.</p>"
    )
    about_body = f"""
...
def update_dashboard(posts: List[Dict[str, Any]]) -> None:
    rows = "\n".join(
        f'<tr><td>{p["date"]}</td><td>{p["title"]}</td><td>{p.get("content_format_name", "")}</td>'
        f'<td><a href="{p["file"]}">보기</a></td></tr>' for p in posts
    )
...
# =====================================================================
# [NEW] Threads 자동 발행 (Blogger 발행 성공 후)
...
def _sns_caption(article: Dict[str, Any], blogger_url: str, *, platform: str) -> str:
    ...
    if title:
        lines.append(title)
    lines.append("자세한 뉘앙스와 예문은 블로그에서 확인하세요.")
    lines.append("Learn Korean → understand how Koreans think & speak.")
    tags = ["#LearnKorean", "#KoreanLanguage", "#한국어", "#KoreanCulture"]
    if cat:
...
def publish_to_sns(article: Dict[str, Any], blogger_url: str, image_url: str) -> None:
    """Blogger 발행 성공 후: Threads에 썸네일 이미지로 공유 (토큰 미설정 시 내부에서 스킵)."""
    if not blogger_url:
        return
...
def _repair_blogger_hero_image(html_content: str, local_thumb_path: str, title: str, public_thumb_url: str = "", expression: str = "") -> str:
    """Blogger 본문 첫 이미지/깨진 히어로를 공개 URL 이미지(+data-URI 안전망)로 교체하고
    alt 텍스트에 로마자 SEO 키워드를 반영한다."""
    if not html_content:
        return html_content
    hero = _blogger_hero_img_html(public_thumb_url, local_thumb_path, title, expression)
...
def _blogger_hero_img_html(thumb_url: str, local_thumb_path: str, title: str, expression: str = "") -> str:
    ...
    try:
        ...
        logger.warning(f"[블로거] 히어로 data-URI 안전망 준비 실패: {e}")
    if data_uri:
...
def publish_to_blogger(article: Dict[str, Any], canonical_url: str, thumb_url: str, local_thumb_path: str) -> Optional[str]:
    ...
    try:
        ...
        content_html = (
            ...
            f'{_blogger_hero_img_html(thumb_url, local_thumb_path, article.get("title") or "", article.get("expression") or "")}'
...
def ensure_ads_txt() -> None:
    """[NEW-애드센스 승인] docs/ads.txt 자동 생성.
    ads.txt 누락은 AdSense 승인 지연·수익 손실의 흔한 원인 중 하나다(특히 GitHub Pages
    커스텀 도메인처럼 자산을 서빙하는 도메인이 별도로 있을 때). ADSENSE_CLIENT_ID가
    'ca-pub-XXXXXXXXXXXXXXXX' 형식일 때만 생성한다.
    ※ 이 블로그의 메인 발행처는 Blogger이며, Blogger 자체 도메인(blogspot.com 또는 연결한
    커스텀 도메인)의 ads.txt는 Blogger 설정 > 수익창출 탭의 토글로 별도 관리해야 한다.
    (Blogger API가 ads.txt를 직접 제어하는 공개 엔드포인트를 제공하지 않아 이 스크립트가
    자동화할 수 없는 영역이다 — 최초 1회 수동 확인 필요.)"""
    if not ADSENSE_CLIENT_ID or not ADSENSE_CLIENT_ID.startswith("ca-pub-"):
        return
    pub_id = ADSENSE_CLIENT_ID.replace("ca-", "", 1)
    content = f"google.com, {pub_id}, DIRECT, f08c47fec0942fa0\n"
    path = os.path.join(DOCS_DIR, "ads.txt")
    try:
        os.makedirs(DOCS_DIR, exist_ok=True)
        existing = ""
        if os.path.exists(path):
        ...
        if existing.strip() != content.strip():
            with open(path, "w", encoding="utf-8") as f:
                f.write(content)
            logger.info(f"[애드센스] docs/ads.txt 생성/갱신 완료 (pub_id={pub_id})")
    except Exception as e:
...
def log_adsense_readiness() -> None:
    ...
    checks = [
        ...
        ("Blogger 발행 자격증명 설정", _blogger_configured()),
        ("리뷰 모드(광고 미노출)", ADSENSE_REVIEW_MODE),
        ("광고 슬롯 설정(승인 후 사용)", bool(ADSENSE_SLOT_IDS)),
        ("최소 본문 품질 하드 게이트", True),
    ]
    stage = "심사 준비 모드(광고 없음)" if ADSENSE_REVIEW_MODE else f"수익화 모드(최대 {ADSENSE_MAX_AD_SLOTS}슬롯)"
    summary = " / ".join(f"{name}={'✅' if ok else '⚠️미설정'}" for name, ok in checks)
    logger.info(f"[애드센스 준비 상태] 현재 단계: {stage} | {summary}")
    if not ADSENSE_REVIEW_MODE and not ADSENSE_SLOT_IDS:
        logger.warning("[애드센스] 리뷰 모드가 꺼져 있는데 광고 슬롯이 없습니다 — 수익화가 되지 않고 있을 수 있습니다. ADSENSE_SLOT_ID(S)를 설정하세요.")
    if ADSENSE_CLIENT_ID and not ADSENSE_CLIENT_ID.startswith("ca-pub-"):
        logger.warning(f"[애드센스] ADSENSE_CLIENT_ID 형식이 'ca-pub-'로 시작하지 않습니다: {ADSENSE_CLIENT_ID!r}")
...
# =====================================================================
_SEO_TITLE_PATTERNS = [
    '"{expr}" ({roman}) Meaning in Korean: What Does It Really Mean?',
    'What Does "{expr}" ({roman}) Mean? Korean Expression Explained',
    '"{expr}" ({roman}) Meaning: What Koreans Actually Mean',
    'Why "{expr}" ({roman}) Has No Direct English Translation',
    '"{expr}" ({roman}) Meaning in Korean — Usage & Cultural Nuance',
]
...
def _seo_rewrite_title(old_title: str, expression: str = "") -> str:
    ...
    new_title = _seo_title_text(pattern, expr)
    # 과도하게 긴 제목 방지
    if len(new_title) > 90:
        roman = romanize_korean(expr)
        suffix = f" ({roman})" if roman else ""
        new_title = f'What Does "{expr}"{suffix} Mean? Korean Expression Explained'
    return new_title
...
def build_ab_title_variants(article: Dict[str, Any]) -> Dict[str, Any]:
    ...
    title_b = _seo_title_text(pattern_b, expr)
    if title_b == title_a:
        pattern_b = _SEO_TITLE_PATTERNS[(h + 1) % len(_SEO_TITLE_PATTERNS)]
        title_b = _seo_title_text(pattern_b, expr)
    if len(title_b) > 90:
        roman_b = romanize_korean(expr)
        suffix_b = f" ({roman_b})" if roman_b else ""
        title_b = f'What Does "{expr}"{suffix_b} Mean? Korean Expression Explained'
...
# =====================================================================
# =====================================================================
# [NEW-위반 리스크 자동 리페어] 과거 Blogger 글이 AdSense 최소 품질 기준(얇은 본문 등)에
# 미달할 경우, Gemini로 완전히 새 글을 생성해 본문 전체를 교체한다.
# GEMINI_API_KEY가 없거나 재생성본마저 품질 기준을 통과하지 못하면 None을 반환하며,
# 이 경우 호출부(repair_old_posts)가 위반 항목을 큐에 기록해 가시적으로 남긴다.
# =====================================================================
def _attempt_violation_regeneration(
    ...
    if not GEMINI_API_KEY or not expr:
        return None
    try:
        fmt = pick_next_content_format(load_queue())
        fresh = generate_article(expr, fmt)
        fresh = fix_character_count_claims(fresh)
        fresh["html_body"] = _soften_culture_claims(fresh.get("html_body", ""))
        ok_fresh, reason_fresh = validate_article_quality(fresh)
        if not ok_fresh:
            logger.warning(f"[위반 리스크] 재생성본도 품질 미달({reason_fresh}) — 자동 교체 보류: {expr}")
            return None

        fresh["category"] = category or fresh.get("category", "번역감정")
        fresh = add_editorial_footer(fresh, seed=seed)
        theme_fresh = get_theme(fresh["category"])
        ...
        footer_html = _blogger_site_footer_html(blog_url, page_urls, seed=seed)
        seo_h1_html = (
            f'<h1 class="seo-post-h1" style="font-size:1.05em;font-weight:600;color:#666;'
            f'margin:0 0 6px;line-height:1.4;">{html.escape(fresh["title"], quote=True)}</h1>'
        )
        hero_html = _blogger_hero_img_html(thumb_pub, local_thumb_abs, fresh["title"], expr)
        tts_html = _tts_buttons_html(expr, theme_fresh)
        badge_html = (
            f'<span style="display:inline-block;background:{theme_fresh["accent"]};color:#fff;'
            f'font-size:0.85em;font-weight:bold;padding:4px 12px;border-radius:999px;margin:14px 0 4px;">'
            f'{theme_fresh["badge"]}</span>'
        )
        new_content = (
            _translate_widget() + nav_html + seo_h1_html + value_html +
            f'<div style="position:relative;margin:0;">{hero_html}{tts_html}</div>' +
            badge_html + _make_blogger_safe_html(fresh["html_body"]) + footer_html
        )
        return fresh["title"], new_content, fresh.get("meta_description", "")
    except Exception as e:
        logger.warning(f"[위반 리스크] 자동 재생성 실패({expr}): {_mask_secrets(str(e))}")
        return None
...
def repair_old_posts() -> None:
    ...
    logger.info(
        f"[복구] GitHub Pages 완료 — 썸네일 재생성 {fixed_thumbs}개(스킵 {skipped_thumbs}개), 발음버튼 {fixed_buttons}개 패치, "
    ...
    if _blogger_configured():
        try:
            ...
            orphan_hold_fixed = 0
            seo_title_fixed = 0
            violation_regenerated = 0
            violation_flagged = 0
            flagged_violations: List[Dict[str, str]] = []
            for bp in blogger_posts:
                ...
                if local:
                    ...
                    if local_thumb and os.path.isfile(local_thumb):
                        ...
                        new_content = _repair_blogger_hero_image(
                            new_content, local_thumb, new_bp_title or bp_title, pub, expr_for_box
                        )
                ...
                # [NEW-위반 리스크 자동 리페어] 얇은 본문(AdSense 최소 품질 기준 미달) 자동 감지.
                # 가능하면 Gemini로 전체 재생성해 교체하고, 불가능하면 큐(flagged_violations)에
                # 기록해 다음 확인 때 놓치지 않도록 남긴다.
                plain_text_check = _strip_html_text(new_content)
                if len(plain_text_check) < ADSENSE_MIN_BODY_CHARS:
                    logger.warning(
                        f"[위반 리스크] 본문 {len(plain_text_check)}자 < 최소 {ADSENSE_MIN_BODY_CHARS}자: {bp_title}"
                    )
                    ...
                    if fix_result:
                        ...
                        if fresh_meta:
                            search_description = fresh_meta[:147].rstrip() + "..." if len(fresh_meta) > 150 else fresh_meta
                        violation_regenerated += 1
                        logger.info(f"[위반 리스크] 자동 재생성 완료: {bp_title} → {len(_strip_html_text(new_content))}자")
                    else:
                        violation_flagged += 1
                        flagged_violations.append({
                            "title": bp_title,
                            "url": bp.get("url", ""),
                            "reason": f"본문 {len(plain_text_check)}자 미달, 자동 재생성 불가(수동 확인 필요)",
                            "date": now_kst().strftime("%Y-%m-%d"),
                        })
            ...
            # [NEW-위반 리스크] 자동 재생성 실패분을 큐에 누적 기록 (최근 100건만 유지, 중복 제목 최신화)
            if flagged_violations:
                vq = load_queue()
                existing_flags = {f.get("title"): f for f in (vq.get("quality_flags") or [])}
                for item in flagged_violations:
                    existing_flags[item["title"]] = item
                vq["quality_flags"] = list(existing_flags.values())[-100:]
                save_queue(vq)
            ...
            logger.info(
                ...
                f"[위반 리스크] 자동 재생성 {violation_regenerated}개, 수동 확인 필요(큐에 기록) {violation_flagged}개)"
            )
        except Exception as e:
...
def run() -> None:
    ...
    # [FIX-핵심] 자동 리페어를 발행 게이트보다 먼저 실행한다.
    # [문제] 기존에는 이 호출이 "check_daily_limit()/should_publish_now_random()" 게이트를
    # 통과한 뒤에만 실행되어, 하루 한도 소진·랜덤 미당첨으로 조기 return되는 대부분의 시간대에는
    # 리페어 자체가 아예 실행되지 않았다. 크론이 매시 정각 돌아도 실제 "발행"은 하루 2회뿐이라,
    # 사실상 리페어가 "발행이 성공한 시간대"에만 우연히 곁다리로 돌아가는 구조였다 (수동 repair
    # 실행에만 사실상 의존하게 된 원인). 발행 게이트와 무관하게 매 실행마다(하루 1회 한도는
    # _maybe_auto_repair_once 내부의 날짜 마커로 여전히 지켜짐) 반드시 리페어 기회를 준다.
    ...
    log_adsense_readiness()  # [NEW-애드센스] 매 실행마다 승인/수익화 준비 상태 요약 로그

    ensure_nojekyll()
    ensure_ads_txt()  # [NEW-애드센스] docs/ads.txt 자동 생성/갱신
    ensure_brand_assets()
    ...
    try:
        chosen_format = select_content_format()
    except Exception as e:
        logger.warning(f"[포맷 선택] 실패, 기본 형식(narrative)로 대체: {e}")
        chosen_format = CONTENT_FORMATS[0]

    try:
        article = generate_article(title, chosen_format)
    except Exception as e:
        logger.error(f"[Gemini] 글 생성 실패 → 로컬 폴백 사용: {e}")
        article = _fallback_article_local(title)
        article["content_format"] = chosen_format.get("id", "")
        article["content_format_name"] = chosen_format.get("name", "")
    ...
    if not ok:
        ...
        content_quality_ok = ok2
        content_quality_reason = reason2
        if not ok2:
            logger.warning(f"[품질] 재생성 후에도 미달({reason2}) — 저품질 상태로 계속 진행하되, Blogger 공개 발행은 아래에서 차단됩니다")
        else:
            logger.info("[품질] 재생성 후 기준 통과")

    # [A/B 제목] SEO 2안 생성·선택 + queue 로그 (장기 성과 추적)
    article = build_ab_title_variants(article)
    # [시각 품질 점수] 본문 기반 점수 산출 (대시보드 추적용)
    ...
    if not content_quality_ok:
        ...
        # 문화 단정 잔존 등)를 통과하지 못한 글은 절대 Blogger에 공개 발행하지 않는다.
        # 애드센스 심사·운영 기간에 저품질/얇은 페이지가 실제 블로그에 노출되는 것을 막기 위함.
        # 썸네일·내부 기록(posts.json, dashboard)은 남겨 다음 실행에서 상태를 추적할 수 있게 한다.
        logger.error(
            f"[품질 게이트] 품질 미달({content_quality_reason})로 Blogger 공개 발행을 건너뜁니다. "
            "(내부 기록만 저장, 실제 블로그에는 게시되지 않음)"
        )
        blogger_url = None
    else:
    ...
    if blogger_url:
        ...
        # [NEW] Threads 자동 공유 (토큰 없으면 내부에서 스킵)
