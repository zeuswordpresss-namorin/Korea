...
# 본문 중간에 삽입되는 이미지 전용 - 카테고리별 장면(scene) 설명
# (요청하신 콜라주 프롬프트의 [SCENE] 자리에 이 설명이 들어갑니다)
SCENE_DESCRIPTIONS = {
...
CONTENT_IMAGE_OPACITY = 0.10  # 프롬프트의 "opacity 10%"는 AI가 정확히 못 지킬 수 있어 코드에서 직접 합성으로 보장
...
def _fetch_content_photo(category: str, seed: int, size=CONTENT_IMAGE_SIZE):
    ...
    찢어붙인 종이(콜라주) 스타일의 흑백 일러스트로 요청하고, 받은 이미지를 흰 배경 위에
    10% 투명도로 합성해서 은은한 텍스처처럼 보이게 만듭니다 (프롬프트의 "opacity 10%" 문구는
    이미지 생성 AI가 정확히 지키지 않는 경우가 많아, 코드에서 직접 합성해 확실히 보장합니다)."""
    ...
    try:
        ...
        # 흰 배경 위에 낮은 투명도로 합성 (요청하신 "opacity 10%" 반영)
...
def insert_content_image(article: dict, slug: str) -> dict:
    """본문 첫 소제목(H2) 바로 뒤에 주제와 관련된 찢어붙인 종이(콜라주) 스타일 이미지를
    1장 삽입합니다 (흰 배경 위 10% 투명도로 은은하게 합성됨)."""
