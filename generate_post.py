...
def render_index_html(posts: List[Dict[str, Any]]) -> None:
    """[NEW] posts 리스트를 받아 index.html을 다시 그린다. update_index()가 새 글 추가 시 쓰는
    공통 렌더링 로직."""
    os.makedirs(DOCS_DIR, exist_ok=True)
...
def _extract_expression_from_title(title: str, strict: bool = False) -> str:
    ...
    3) [strict=False일 때만] 그래도 못 찾으면 제목 전체에서 첫 한글 어절을 최후 수단으로 추출.
       ...
       아무 한글이나 집어 정상 추출된 것처럼 오판하는 원인이었다. "이 글이 애초에 표현 글이
       맞는지"를 판별해야 하는 곳에서는 strict=True로 3번 단계를 건너뛴다.
    """
    if not title:
        return ""
